...
BITS_PER_ELEMENT="${BITS_PER_ELEMENT:-16}"
...
BIT_LEN="$(next_pow2 $((NUM_ELE * BITS_PER_ELEMENT)))"

STAMP="$(date +%Y%m%d_%H%M%S)"
RESULTS="$RESULT_DIR/hash_count_${STAMP}.tsv"
...
run_case() {
  local hash_num="$1"
  local layout="$2"
  local run_tag="h${hash_num}_${layout}_b${BIT_LEN}"
  ...
  echo -e "hash_count\t$hash_num\t$layout\t$BIT_LEN\t$hash_num\t$maker_sec\t$maker_rss\t$insert_ops\t$insert_ns\t$cat_sec\t$cat_rss\t$query_ops\t$query_ns" >> "$RESULTS"
}

IFS=',' read -r -a HASH_VALUES <<< "$HASH_LIST"
for hash_num in "${HASH_VALUES[@]}"; do
  hash_num="${hash_num// /}"
  [[ -n "$hash_num" ]] || continue
  run_case "$hash_num" classic
  run_case "$hash_num" blocked
done
