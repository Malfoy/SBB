...
BENCH_ROOT="${BENCH_ROOT:-/tmp/sbb-single-ref-baseline}"
...
PLOT_SCRIPT="${PLOT_SCRIPT:-$ROOT_DIR/bench/plot_single_ref_baseline.py}"
...
echo "[baseline] building binaries"
cargo build --release --bin sbb --bin sbbbenchgen >/tmp/sbb_single_ref_baseline_build.log
...
if [[ ! -s "$DATA_DIR/ref.fa" || ! -s "$DATA_DIR/reads.fa" || "$CURRENT_META"$'\n' != "$EXPECTED_META" ]]; then
  if [[ -n "$REFERENCE_FASTA" ]]; then
    echo "[baseline] preparing dataset from reference: $REFERENCE_FASTA"
    ...
    echo "[baseline] generating synthetic dataset"
    "$BENCHGEN" \
      --out_dir "$DATA_DIR" \
      --ref_len "$REF_LEN" \
      --read_len "$READ_LEN" \
      ...
      --seed "$SEED" \
      --threads "$THREADS" \
      >/tmp/sbb_single_ref_baseline_benchgen.log
  fi
...
RESULTS="$RESULT_DIR/single_ref_baseline_${STAMP}.tsv"
echo -e "benchmark\ttool\tlayout\trepeat\tthreads\tkmer_size\tfpr\tscore\tref_len\tread_len\tread_count\tbuild_sec\tbuild_rss_kb\tquery_sec\tquery_rss_kb\tindex_bytes\tinsert_kmer_ops\tinsert_mean_ns_per_kmer\tquery_kmer_queries\tquery_mean_ns_per_kmer\treads_per_sec\tquery_kmers_per_sec\tfilter_path\treads_path" > "$RESULTS"
...
run_case() {
  ...
  echo -e "single_ref_baseline\t$tool\t$layout\t$rep\t$THREADS\t$KMER_SIZE\t$FPR\t$CAT_SCORE\t$REF_LEN\t$READ_LEN\t$READ_COUNT\t$build_sec\t$build_rss\t$query_sec\t$query_rss\t$index_bytes\t$insert_ops\t$insert_ns\t$query_ops\t$query_ns\t$reads_per_sec\t$query_kmers_per_sec\t$filter_path\t$READS_INPUT" >> "$RESULTS"
}
...
if [[ "$PLOT" == "1" ]]; then
  if command -v python3 >/dev/null 2>&1; then
    ...
    echo "[baseline] python3 not found; skipping plots" >&2
  fi
fi
