...
BENCH_ROOT="${BENCH_ROOT:-/tmp/sbb-single-ref-sweep}"
...
SWEEP_MODES="${SWEEP_MODES:-fpr,hash,block_words}"
FPR_LIST="${FPR_LIST:-0.01,0.005,0.001,0.0001,0.00001}"
HASH_LIST="${HASH_LIST:-1,2,3,4,6,8}"
BLOCK_WORDS_LIST="${BLOCK_WORDS_LIST:-1,2,4,8,16}"
FIXED_FPR="${FIXED_FPR:-0.00001}"
FIXED_HASH="${FIXED_HASH:-4}"
FIXED_BLOCK_WORDS="${FIXED_BLOCK_WORDS:-8}"
...
PLOT_SCRIPT="${PLOT_SCRIPT:-$ROOT_DIR/bench/plot_single_ref_sweep.py}"
...
echo "[sweep] building binaries"
cargo build --release --bin sbb --bin sbbbenchgen >/tmp/sbb_single_ref_sweep_build.log
...
if [[ ! -s "$DATA_DIR/ref.fa" || ! -s "$DATA_DIR/reads.fa" || "$CURRENT_META"$'\n' != "$EXPECTED_META" ]]; then
  if [[ -n "$REFERENCE_FASTA" ]]; then
    echo "[sweep] preparing dataset from reference: $REFERENCE_FASTA"
    ...
    echo "[sweep] generating synthetic dataset"
    "$BENCHGEN" \
      --out_dir "$DATA_DIR" \
      --ref_len "$REF_LEN" \
      --read_len "$READ_LEN" \
      ...
      --seed "$SEED" \
      --threads "$THREADS" \
      >/tmp/sbb_single_ref_sweep_benchgen.log
  fi
...
RESULTS="$RESULT_DIR/single_ref_sweep_${STAMP}.tsv"
echo -e "benchmark\tsweep\tparam\ttool\tlayout\trepeat\tthreads\tkmer_size\tfpr\thash_num\tblock_words\tscore\tbuild_sec\tbuild_rss_kb\tquery_sec\tquery_rss_kb\tindex_bytes\tinsert_kmer_ops\tinsert_mean_ns_per_kmer\tquery_kmer_queries\tquery_mean_ns_per_kmer\treads_per_sec\tquery_kmers_per_sec\tfilter_path\treads_path" > "$RESULTS"
...
run_case() {
  ...
  local hash_num="$7"
  local block_words="$8"
  local run_tag="${sweep}_${param}_${tool}_${layout}_r${rep}"
  ...
  echo -e "single_ref_sweep\t$sweep\t$param\t$tool\t$layout\t$rep\t$THREADS\t$KMER_SIZE\t$fpr\t$hash_num\t$block_words\t$CAT_SCORE\t$build_sec\t$build_rss\t$query_sec\t$query_rss\t$index_bytes\t$insert_ops\t$insert_ns\t$query_ops\t$query_ns\t$reads_per_sec\t$query_kmers_per_sec\t$filter_path\t$READS_INPUT" >> "$RESULTS"
}
...
run_mode() {
  ...
  case "$mode" in
    fpr)
      IFS=',' read -r -a params <<< "$FPR_LIST"
      for param in "${params[@]}"; do
        param="${param// /}"
        [[ -n "$param" ]] || continue
        for rep in $(seq 1 "$REPEATS"); do
          run_case "$mode" "$param" cpp classic "$rep" "$param" "$FIXED_HASH" "$FIXED_BLOCK_WORDS"
          run_case "$mode" "$param" rust classic "$rep" "$param" "$FIXED_HASH" "$FIXED_BLOCK_WORDS"
          run_case "$mode" "$param" rust blocked "$rep" "$param" "$FIXED_HASH" "$FIXED_BLOCK_WORDS"
        done
      done
      ;;
    hash)
      IFS=',' read -r -a params <<< "$HASH_LIST"
      for param in "${params[@]}"; do
        param="${param// /}"
        [[ -n "$param" ]] || continue
        for rep in $(seq 1 "$REPEATS"); do
          run_case "$mode" "$param" cpp classic "$rep" "$FIXED_FPR" "$param" "$FIXED_BLOCK_WORDS"
          run_case "$mode" "$param" rust classic "$rep" "$FIXED_FPR" "$param" "$FIXED_BLOCK_WORDS"
          run_case "$mode" "$param" rust blocked "$rep" "$FIXED_FPR" "$param" "$FIXED_BLOCK_WORDS"
        done
      done
      ;;
    block_words)
      IFS=',' read -r -a params <<< "$BLOCK_WORDS_LIST"
      for param in "${params[@]}"; do
        param="${param// /}"
        [[ -n "$param" ]] || continue
        for rep in $(seq 1 "$REPEATS"); do
          run_case "$mode" "$param" cpp classic "$rep" "$FIXED_FPR" "$FIXED_HASH" "$param"
          run_case "$mode" "$param" rust classic "$rep" "$FIXED_FPR" "$FIXED_HASH" "$param"
          run_case "$mode" "$param" rust blocked "$rep" "$FIXED_FPR" "$FIXED_HASH" "$param"
        done
      done
      ;;
    *)
...
IFS=',' read -r -a MODES <<< "$SWEEP_MODES"
...
if [[ "$PLOT" == "1" ]]; then
  if command -v python3 >/dev/null 2>&1; then
    ...
    echo "[sweep] python3 not found; skipping plots" >&2
  fi
fi
