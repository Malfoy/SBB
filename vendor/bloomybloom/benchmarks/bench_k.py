...
def build_command(index_file: str, query_file: str, k_value: int) -> list[str]:
    command = [
...
def plot_rows(rows: list[dict[str, object]], output_path: Path) -> None:
    x_values = [int(row["k"]) for row in rows]
    ...
    fig.suptitle("bloomybloom benchmark: k sweep")
    fig.tight_layout()
...
def main() -> None:
    ...
    for k_value in K_VALUES:
        metrics_list = [
            run_bloom(root, build_command(args.index_file, args.query_file, k_value))
