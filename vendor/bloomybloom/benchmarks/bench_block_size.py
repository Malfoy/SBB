...
BENCHMARK_NAME = "bench_block_size"
BLOCK_SIZE_EXPONENTS = [10, 11, 12, 13, 14, 15]
K_VALUE = 31
M_VALUE = 13
S_VALUE = 28
N_HASHES = 3
...
def build_command(index_file: str, query_file: str, block_size_exponent: int) -> list[str]:
    command = [
...
def plot_rows(rows: list[dict[str, object]], output_path: Path) -> None:
    x_values = [int(row["block_size_exponent"]) for row in rows]
    ...
    axes[0].set_xlabel("block size exponent")
    ...
    axes[1].set_xlabel("block size exponent")
    axes[1].set_ylabel("seconds")
    axes[1].grid(True, alpha=0.3)
    axes[1].legend()

    fig.suptitle("bloomybloom benchmark: block-size sweep")
    fig.tight_layout()
...
def main() -> None:
    ...
    for block_size_exponent in BLOCK_SIZE_EXPONENTS:
        metrics_list = [
            run_bloom(root, build_command(args.index_file, args.query_file, block_size_exponent))
